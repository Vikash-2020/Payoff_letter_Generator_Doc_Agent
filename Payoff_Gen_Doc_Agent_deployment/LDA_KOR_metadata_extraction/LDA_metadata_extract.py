...
# from KOR_metadata_extraction import process_document_chunks
...
# def normalize_text(text):  
#     # Normalize whitespace to single space and strip leading/trailing whitespace  
#     text = re.sub(r'\s+', ' ', text).strip()  
      
#     # Combine multiple replacements with character classes and pipe for alternatives  
#     text = re.sub(r"[,;:\[\]“”\"']|(\.\s*,)|(\.\s*\.)", "", text)  
  
#     # Remove extra periods and spaces around periods  
...
def extract_keywords_from_nodes(nodes, num_topics=1, words_per_topic=15):  
    ...
    for node in nodes:  
        try:  
            ...
            # kor_metadata = extract_metadata_from_chunk(node.text)
            # metadata = {**metadata, **kor_metadata}
...
# # Define the path to the documents folder and the storage folder  
# doc_folder_path = "./NPGA"
# storage_folder_path = "./MNX_LDA_metadata_vector_store"

# process_documents(doc_folder=doc_folder_path, storage_base_folder= storage_folder_path)
